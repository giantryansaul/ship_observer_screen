...
| Top half or bottom half is dark | Wrong row-scan/multiplexing for the panel. Add `--led-multiplexing=1` (then 2, 3...) to the `demo` command to find the right value. **This service does not currently expose a multiplexing setting** - `MATRIX_ROWS` controls panel row count, not scan pattern, and will not fix this. If your panel needs a non-zero value, it must be added to `RgbMatrixDriver` in `ship_observer/drivers/rgbmatrix.py` (`options.multiplexing = <value>`) before the service will display correctly - most common panels use the default (0) and never hit this. |
